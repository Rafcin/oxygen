...
export function useGeolocated(config: GeolocatedConfig = {}): GeolocatedResult {
  ...
  const userDecisionTimeoutId = React.useRef(0)
  const isCurrentlyMounted = React.useRef(true)
  const watchId = React.useRef<number | void>(0)

  const [isGeolocationEnabled, setIsGeolocationEnabled] = React.useState(
  ...
  const handlePositionSuccess = React.useCallback(
